...
from retro_paths import RETRO_UPDATE, RETRO_LOGS_FOLDER, TODAY
...
def rsf_templates():
    ...
    def bodyHtmlSunOrOptical(row):
        model = row["Title"]
        # SOLE
        anchor_text_sun = "Retrosuperfuture sunglasses 2024 | 2025"
        url_sun = "https://lookeronline.com/collections/retrosuperfuture-sunglasses"
        # VISTA
        anchor_text_eye = "Retrosuperfuture Eyeglasses 2024 | 2025"
...
if __name__ == "__main__":
    with open(RETRO_LOGS_FOLDER, "a") as file:
        try:
            print("Updating RetroSuperFuture templates...")
            rsf_templates()
            print("Templates RetroSuperFuture updated.")
            file.write(
                f"RSF TEMPLATES: RSF log of {TODAY}: RetroSuperFuture templates are updated successfully! \n"
            )
        except Exception as err:
            print(f"Templates was not updated due to this error. {type(err).__name__}: {err}")
            file.write(
                f"RSF TEMPLATES: RSF log of {TODAY}: RetroSuperFuture templates items not updated due this error: - {type(err).__name__}: {err} \n"
            )

...
from luxottica_paths import vogue_folder, vogue_new, logs_folder, to_import_images

def get_vogue_images():

    # Carica il dataframe e definisci la cartella immagini
    df = pd.read_excel(vogue_new)
    ...
    def get_spinimages_tag(row):
        if "Spinimages" not in row['Tags'].lower():
            images = str(row['Image Src']).split(";")
            number_images = len(images)
            return f"{row['Tags']}, spinimages={number_images}"
        return row['Tags']
    ...
    def check_df_is_empty(df):
        if df.empty:
            print("Vogue Eyewear doesn't have Images")
        else:
            df.to_excel(f"{vogue_folder}/Vogue_IMG_OK.xlsx", index=False)
            df.to_excel(f"{to_import_images}/vogue_img_ok.xlsx", index = False)

            # Rename the sheet to "Products
            wb = openpyxl.load_workbook(f"{to_import_images}/vogue_img_ok.xlsx")
            ws = wb.active
            ws.title = "Products"
            wb.save(f"{to_import_images}/vogue_img_ok.xlsx")

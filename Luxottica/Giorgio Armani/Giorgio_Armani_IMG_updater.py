...
from luxottica_paths import giorgio_armani_folder, giorgio_armani_new, logs_folder, to_import_images

def get_giorgio_armani_images():

    # Carica il dataframe e definisci la cartella immagini
    df = pd.read_excel(giorgio_armani_new)
    ...
    def generate_image_urls(row):
        ...
        suffixes = ["", ".jpeg","__P21__shad__fr.png", "__P21__shad__al2.png", "__P21__shad__al3.png", "__P21__shad__bk.png",
                    ...
                    "__STD__shad__bk.png", "__STD__shad__cfr.png", "__STD__shad__fr.png", "__STD__shad__lt.png", "__STD__shad__qt.png"]
    ...
    def check_df_is_empty(df):
        if df.empty:
            print("Giorgio Armani doesn't have Images")
        else:
            df.to_excel(f"{giorgio_armani_folder}/Giorgio_Armani_IMG_OK.xlsx", index=False)
            df.to_excel(f"{to_import_images}/Giorgio_Armani_img_ok.xlsx", index = False)

            # Rename the sheet to "Products
            wb = openpyxl.load_workbook(f"{to_import_images}/Giorgio_Armani_img_ok.xlsx")
            ws = wb.active
            ws.title = "Products"
            wb.save(f"{to_import_images}/Giorgio_Armani_img_ok.xlsx")

    check_df_is_empty(df)

if __name__ == "__main__":
    try:
        print("Getting Giorgio_Armani images for new products...")
        time.sleep(1)
        get_giorgio_armani_images()
        print("Giorgio_Armani done")

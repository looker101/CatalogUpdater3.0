...
from luxottica_paths import michael_kors_folder, michael_kors_new, logs_folder, to_import_images

def get_michael_kors_images():

    # Carica il dataframe e definisci la cartella immagini
    df = pd.read_excel(michael_kors_new)
    ...
    def check_df_is_empty(df):
        if df.empty:
            print("Michael Kors doesn't have Images")
        else:
            df.to_excel(f"{michael_kors_folder}/MK_IMG.xlsx", index=False)
            df.to_excel(f"{to_import_images}/michael_kors_img_ok.xlsx", index = False)

            # Rename the sheet to "Products
            wb = openpyxl.load_workbook(f"{to_import_images}/michael_kors_img_ok.xlsx")
            ws = wb.active
            ws.title = "Products"
            wb.save(f"{to_import_images}/michael_kors_img_ok.xlsx")

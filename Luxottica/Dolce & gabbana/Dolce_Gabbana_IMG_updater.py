...
from luxottica_paths import dolce_gabbana_folder, dolce_gabbana_new, logs_folder, to_import_images

def get_dg_images():

    # Carica il dataframe e definisci la cartella immagini
    df = pd.read_excel(dolce_gabbana_new)
    ...
    def check_df_is_empty(df):
        if df.empty:
            print("Dolce & Gabbana doesn't have images")
        else:
            df.to_excel(f"{dolce_gabbana_folder_folder}/Dolce_Gabbana_IMG_OK.xlsx", index=False)
            df.to_excel(f"{to_import_images}/Dolce_Gabbana_img_ok.xlsx", index = False)

            # Rename the sheet to "Products
            wb = openpyxl.load_workbook(f"{to_import_images}/Dolce_Gabbana_img_ok.xlsx")
            ws = wb.active
            ws.title = "Products"
            wb.save(f"{to_import_images}/Dolce_Gabbana_img_ok.xlsx")

    check_df_is_empty(df)

if __name__ == "__main__":
    try:
        print("Getting Dolce_Gabbana images for new products...")
        time.sleep(1)
        get_dg_images()
        print("Dolce_Gabbana done")

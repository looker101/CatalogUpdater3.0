...
from luxottica_paths import burberry_folder, burberry_new, logs_folder, to_import_images

def get_burberry_images():

    # Carica il dataframe e definisci la cartella immagini
    df = pd.read_excel(burberry_new)
    ...
    df["Check_IMG"] = df["Variant SKU"].fillna("").astype(str).str.strip().str.replace(" ", "__")
    ...
    def check_df_is_empty(df):
        if df.empty:
            print("Burberry doesn't have images")
        else:
            df.to_excel(f"{burberry_folder}/Burberry_IMG_OK.xlsx", index=False)
            df.to_excel(f"{to_import_images}/Burberry_img_ok.xlsx", index = False)

            # Rename the sheet to "Products
            wb = openpyxl.load_workbook(f"{to_import_images}/Burberry_img_ok.xlsx")
            ws = wb.active
            ws.title = "Products"
            wb.save(f"{to_import_images}/Burberry_img_ok.xlsx")

...
from luxottica_paths import ray_ban_folder, ray_ban_new, logs_folder, to_import_images

def get_ray_ban_images():

    # Carica il dataframe e definisci la cartella immagini
    df = pd.read_excel(ray_ban_new)
    ...
    def check_df_is_empty(df):
        if df.empty:
            print("Ray-Ban doesn't have Images")
        else:
            df.to_excel(f"{ray_ban_folder}/RB_IMG_OK.xlsx", index=False)
            df.to_excel(f"{to_import_images}/ray-ban_img_ok.xlsx", index = False)

            # Rename the sheet to "Products
            wb = openpyxl.load_workbook(f"{to_import_images}/ray-ban_img_ok.xlsx")
            ws = wb.active
            ws.title = "Products"
            wb.save(f"{to_import_images}/ray-ban_img_ok.xlsx")

...
from luxottica_paths import emporio_armani_folder, emporio_armani_new, logs_folder, to_import_images

def get_emporio_armani_images():

    # Carica il dataframe e definisci la cartella immagini
    df = pd.read_excel(emporio_armani_new)
    ...
    def check_df_is_empty(df):
        if df.empty:
            print("Emporio Armani doesn't have Images")
        else:
            df.to_excel(f"{emporio_armani_folder}/Emporio_Armani_IMG_OK.xlsx", index=False)
            df.to_excel(f"{to_import_images}/Emporio_Armani_img_ok.xlsx", index = False)

            # Rename the sheet to "Products
            wb = openpyxl.load_workbook(f"{to_import_images}/Emporio_Armani_img_ok.xlsx")
            ws = wb.active
            ws.title = "Products"
            wb.save(f"{to_import_images}/Emporio_Armani_img_ok.xlsx")
        check_df_is_empty(df)

if __name__ == "__main__":
    try:
        print("Getting Emporio_Armani images for new products...")
        time.sleep(1)
        get_emporio_armani_images()
        print("Emporio_Armani done")

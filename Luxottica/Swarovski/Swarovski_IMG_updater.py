...
from luxottica_paths import swarovski_folder, swarovski_new, logs_folder, to_import_images

def get_swarovski_images():

    # Carica il dataframe e definisci la cartella immagini
    df = pd.read_excel(swarovski_new)
    ...
    def check_df_is_empty(df):
        if df.empty:
            print("Swarovski doesn't have Images")
        else:
            df.to_excel(f"{swarovski_folder}/Swarovski_IMG_OK.xlsx", index=False)
            df.to_excel(f"{to_import_images}/Swarovski_img_ok.xlsx", index = False)

            # Rename the sheet to "Products
            wb = openpyxl.load_workbook(f"{to_import_images}/Swarovski_img_ok.xlsx")
            ws = wb.active
            ws.title = "Products"
            wb.save(f"{to_import_images}/Swarovski_img_ok.xlsx")

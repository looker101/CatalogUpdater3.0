...
from luxottica_paths import tiffany_folder, tiffany_new, logs_folder, to_import_images

def get_tiffany_images():

    # Carica il dataframe e definisci la cartella immagini
    df = pd.read_excel(tiffany_new)
    ...
    def check_df_is_empty(df):
        if df.empty:
            print("Tiffany doesn't have Images")
        else:
            df.to_excel(f"{tiffany_folder}/Tiffany_IMG_OK.xlsx", index=False)
            df.to_excel(f"{to_import_images}/Tiffany_img_ok.xlsx", index = False)

            # Rename the sheet to "Products
            wb = openpyxl.load_workbook(f"{to_import_images}/Tiffany_img_ok.xlsx")
            ws = wb.active
            ws.title = "Products"
            wb.save(f"{to_import_images}/Tiffany_img_ok.xlsx")

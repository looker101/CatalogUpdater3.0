...
from luxottica_paths import versace_folder, versace_new, logs_folder, to_import_images

def get_versace_images():

    # Carica il dataframe e definisci la cartella immagini
    df = pd.read_excel(versace_new)
    ...
    def check_df_is_empty(df):
        if df.empty:
            print("Versace doesn't have Images")
        else:
            df.to_excel(f"{versace_folder}/Versace_IMG_OK.xlsx", index=False)
            df.to_excel(f"{to_import_images}/Versace_img_ok.xlsx", index = False)

            # Rename the sheet to "Products
            wb = openpyxl.load_workbook(f"{to_import_images}/Versace_img_ok.xlsx")
            ws = wb.active
            ws.title = "Products"
            wb.save(f"{to_import_images}/Versace_img_ok.xlsx")

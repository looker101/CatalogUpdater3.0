...
from luxottica_paths import arnette_folder, arnette_new,  logs_folder, to_import_images

def get_arnette_images():

    # Carica il dataframe e definisci la cartella immagini
    df = pd.read_excel(arnette_new)
    ...
    df["Check_IMG"] = df["Variant SKU"].str.strip().str.replace(" ", "__")
    df["Check_IMG"] = df["Variant SKU"].str[:-2]
    df["Check_IMG"] = df["Check_IMG"].str.strip().str.replace(" ", "__").str.replace("/","_")
    df["Check_IMG"] = df.apply(lambda row: f'0{row["Check_IMG"]}', axis=1)
    #df["Check_IMG"] = df["Variant SKU"].str.replace('0', '', 1)
    ...
    def generate_image_urls(row):
        ...
        suffixes = ["__P21__shad__qt.png", "", ".jpeg","__P21__shad__fr.png", "__P21__shad__al2.png", "__P21__shad__al3.png", "__P21__shad__bk.png",
                    "__P21__shad__cfr.png", "__P21__shad__lt.png",
                    "_000A.png", "_030A.png", "_060A.png", "_090A.png", "_120A.png", "_150A.png", "_180A.png",
                    "_210A.png", "_240A.png", "_270A.png", "_300A.png", "_330A.png",
                    "__STD__shad__bk.png", "__STD__shad__cfr.png", "__STD__shad__fr.png", "__STD__shad__lt.png", "__STD__shad__qt.png", "__STD__shad__al1"]
    ...
    def check_df_is_empty(df):
        if df.empty:
            print("Arnette doesn't have images")
        else:
            print("Arnette saved successfully!")
            df.to_excel(f"{arnette_folder}/Arnette_IMG_OK.xlsx", index=False)
            df.to_excel(f"{to_import_images}/Arnette_img_ok.xlsx", index = False)

            # Rename the sheet to "Products
            wb = openpyxl.load_workbook(f"{to_import_images}/Arnette_img_ok.xlsx")
            ws = wb.active
            ws.title = "Products"
            wb.save(f"{to_import_images}/Arnette_img_ok.xlsx")

...
from luxottica_paths import persol_folder, persol_new, logs_folder, to_import_images

def get_persol_images():

    # Carica il dataframe e definisci la cartella immagini
    df = pd.read_excel(persol_new)
    ...
    def check_df_is_empty(df):
        if df.empty:
            print("Persol doesn't have Images")
        else:
            df.to_excel(f"{persol_folder}/Persol_IMG_OK.xlsx", index=False)
            df.to_excel(f"{to_import_images}/Persol_img_ok.xlsx", index=False)

            # Rename the sheet to "Products
            wb = openpyxl.load_workbook(f"{to_import_images}/Persol_img_ok.xlsx")
            ws = wb.active
            ws.title = "Products"
            wb.save(f"{to_import_images}/Persol_img_ok.xlsx")
